...
<?php
echo anchor(base_url() . 'beranda/add/', 'Add');
if (!$results) {
    echo '<h1>No Data</h1>';
    exit;
}
...
for ($i = 0; $i < count($results); $i++) {
    ...
    $results[$i]['Edit'] = anchor(base_url() . 'beranda/edit/' . $id[0], 'Edit');
    $results[$i]['Delete'] = anchor(base_url() . 'beranda/delete/' . $id[0], 'Delete', array('onClick' => 'return deletechecked(\' ' . base_url() . 'beranda/delete/' . $id[0] . ' \')'));
