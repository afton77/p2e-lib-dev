...
<h3>FGD</h3>
<?php
// echo anchor(base_url() . 'fgd/add/', 'Add');
if (!$results) {
    echo '<h1>No Data</h1>';
    exit;
}
...
for ($i = 0; $i < count($results); $i++) {
    ...
    $results[$i]['file'] = anchor(base_url() . 'formrequest/request/fgd/' . $results[$i]['id'], 'Request');
    // $results[$i]['Edit'] = anchor(base_url() . 'fgd/edit/' . $id[0], 'Edit');
    // $results[$i]['Delete'] = anchor(base_url() . 'fgd/delete/' . $id[0], 'Delete', array('onClick' => 'return deletechecked(\' ' . base_url() . 'fgd/delete/' . $id[0] . ' \')'));
