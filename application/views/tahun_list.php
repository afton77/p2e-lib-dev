...
<?php
echo anchor(base_url() . 'tahun/add/', 'Add');
if (!$results) {
    echo '<h1>No Data</h1>';
    exit;
}
...
for ($i = 0; $i < count($results); $i++) {
    ...
    $results[$i]['Edit'] = anchor(base_url() . 'tahun/edit/' . $id[0], 'Edit');
    $results[$i]['Delete'] = anchor(base_url() . 'tahun/delete/' . $id[0], 'Delete', array('onClick' => 'return deletechecked(\' ' . base_url() . 'tahun/delete/' . $id[0] . ' \')'));
