...
<?php
echo anchor(base_url() . 'beritaekonomi/add/', 'Add');
if (!$results) {
    echo '<h1>No Data</h1>';
    exit;
}
...
for ($i = 0; $i < count($results); $i++) {
    ...
    $results[$i]['Edit'] = anchor(base_url() . 'beritaekonomi/edit/' . $id[0], 'Edit');
    $results[$i]['Delete'] = anchor(base_url() . 'beritaekonomi/delete/' . $id[0], 'Delete', array('onClick' => 'return deletechecked(\' ' . base_url() . 'beritaekonomi/delete/' . $id[0] . ' \')'));
