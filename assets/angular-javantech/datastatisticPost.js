...
myJavanTech.factory("servicesJT", ['$http', function ($http) {
    ...
    obj.submitData = function (data) {
      Upload.upload({
        url: serviceBase + 'datastatistik/saveData',
        method: 'POST',
        file: file,
        sendFieldsAs: 'form',
        fields: data
    ...
    obj.getCategories = function () {
      return $http.get(serviceBase + 'category/get_cmb_category_datastatistic');
    };

    obj.getData = function (ID) {
      return $http.get(serviceBase + 'bankdata/getData/' + ID + '/ds');
    };
    ...
    if ($location.path().replace().replace('/', '')) {
      ...
      $timeout(function () {
        servicesJT.getData(id).success(function (r) {
          ...
          $scope.cmbKategory = r.json[0].kategori;
          $scope.txtSource = r.json[0].sumber;
        });
      }, 1000);
    }
...
//      console.log(data);
      Upload.upload({
        url: serviceBase + 'datastatistik/save_data',
        method: 'POST',
        file: $scope.file,
        fields: data,
        data: data
        ...
        window.location.href = serviceBase + 'datastatistik/manage/';
...
myJavanTech.config(function ($routeProvider, $locationProvider) {
  $routeProvider.when("/post/:id", {
    title: 'Edit Data',
    templateUrl: '../bankdata/management',
